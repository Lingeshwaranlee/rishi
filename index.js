...
dotenv.config();
 ...
 //const MONGO_URL = "mongodb+srv://lingeshwaran:lingesh150&@cluster0.v9lyg.mongodb.net/";
...
app.get('/',function(req,res){
    res.send("hello this is lingesh ")
})
app.get('/foods',async function(req,res){
const food = await client.db("b30wd").collection("foods").find({}).toArray(); 
    res.send(food)
})
app.get('/foods/:id',async function(request,response){
 ...
 app.delete('/foods/:id',async function(request,response){
  console.log(request.params)
  const {id} = request.params;
  const food = await client.db("b30wd").collection("foods").deleteOne({ id: id }); 
      response.send(food)
  })

app.post("/foods", async function(request,response){
    const data=request.body; 
        const result = await client.db("b30wd").collection("foods").insertMany(data);
        response.send(result)
        });



app.listen(PORT,()=>console.log(`server is startede ${PORT}`));